...
namespace Data.MSSQL
{
    public class UserSource : SourceBase, IUserSource
    {
        ...
        public async Task<User> GetUserAsync(int userId)
        {
            ...
            {
                const string query = "SELECT PSID AS ID, AnredeID AS Salutation, Titel AS Title, Vorname AS NameFirst, Nachname AS NameLast, dwTel AS Phone, dwFax AS Fax, Mail AS EMail " +
                                     "FROM tblPs " +
                                     "WHERE PSID = @id";
        ...
        public async Task<User> GetUserAsync(string email)
        {
            using (var con = new SqlConnection(ConnectionString))
            {
                const string query = "SELECT DISTINCT [user].PSID AS USER_ID, [user].AnredeID AS USER_Salutation, [user].Titel AS USER_Title, [user].Vorname AS USER_NameFirst, [user].Nachname AS USER_NameLast, " +
        ...
        public async Task SaveNewPasswordAsync(string password, int userId)
        {
            using (var con = new SqlConnection(ConnectionString))
            {
                const string query = "IF EXISTS(SELECT 1 FROM tblChangedPasswords WHERE PsID = @userId) " +
